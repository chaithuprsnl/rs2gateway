...
public class RS2ISOMessage {
	...
	public byte[] pack() {
		...
		try {
			...
			getBitmapFieldAtt(msgMap, fieldMessage, actualBitMap, env);
	...
	protected void splitMessage(String message, Map<String, String> isoBuffer, String bitMap, int offset) {
		...
		for(int i=0; i<bitMap.length(); i++) {
			...
			case '1':
				String propVal = String.valueOf(env.getProperty("F-" + (i+1)));
		...
		try {
			...
			for (int i = 0; i < fieldlength; i++) {
				...
				if (bitDetails != null) {
					...
					String subElement = env.getProperty("S-" + bitPos + "." + subElementPos);
